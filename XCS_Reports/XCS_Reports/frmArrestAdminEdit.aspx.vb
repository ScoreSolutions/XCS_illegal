    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            If Request("UserName") IsNot Nothing Then
                GetLoginHisID(Request("UserName"))
                SaveLogTrans(Request("UserName"), "คลิกเมนู แก้ไขข้อมูลคดี")
            End If
        End If
...
55:
    ...
    Function Update(ByVal UserName As String) As Boolean
        Dim trans As New TransactionDB
                        ...
                        If ret = True Then
                            ...
                            If ret = True Then
                                ...
                                If lblLoginHisID.Text <> "0" Then
                                    SaveLogTrans(UserName, "แก้ไขข้อมูลคดี เลขที่งาน " & txtTrack_no.Text)
                                End If
                            Else
                                trans.RollbackTransaction()
                            End If
    ...
    Function GetDataFromArrest(ByVal track_no As String) As DataTable
        Dim sql As String = "select aa.lawbreak_type, aa.scene_name, aa.scene_no, aa.scene_moo, aa.scene_soi, aa.scene_road, aa.police_station, "
        sql += " aa.accuser_poscode, aa.accuser_offcode, aa.issentcourt, aa.estimate_fine, aa.department_id, aa.department_name, "
        sql += " aa.id, aa.track_no, pke_arrest.getArrestIndictment(aa.track_no) guilt_base, aa.coordinate_x, aa.coordinate_y, "
        sql += " aa.accuser_idcardno, aa.report_to ,aa.order_by, aa.SCENE_PROVINCE, aa.SCENE_SUBDISTRICT, aa.SCENE_DISTRICT,"
        sql += " aa.occurrence_date, to_char(aa.occurrence_date, 'dd/mm/yyyy', 'NLS_CALENDAR=''THAI BUDDHA'' NLS_DATE_LANGUAGE=THAI') occurrence_date2,  "
        sql += " to_char(aa.occurrence_date,'HH24:mi') occurrence_time, "
        sql += " aa.application_date, to_char(aa.application_date, 'dd/mm/yyyy', 'NLS_CALENDAR=''THAI BUDDHA'' NLS_DATE_LANGUAGE=THAI')  application_date2, "
        sql += " nvl(aa.accuser_name,tia.title_name || sta.first_name || ' ' || sta.last_name) accuser_name, aa.legislation_id, lg.legislation_name, "
        sql += " aa.product_group_id, dg.group_name product_group_name, aa.accuser_type,  aa.have_culprit, aa.offcode, ed.short_name area_offname, "
        sql += " nvl(aa.accuser_offname, nvl(sta.offname,eda.offname)) offname,nvl(aa.accuser_posname, nvl(sta.posname,ps.posname)) posname,  "
        sql += " ana.notice_no , ana.inform_name ,asw.warrant_no, asi.track_no confirm_no,aa.application_notice_arrest_id,aa.application_search_warrant_id, "
        sql += " aa.application_sincere_confirm_id, nvl(aa.report_to_staff,tir.title_name || str.first_name || ' ' || str.last_name) report_to_staff,   "
        sql += " tio.title_name || sto.first_name || ' ' || sto.last_name order_by_staff, pke_compare.chkCaseLast(aa.track_no) chkCaseLast,  "
        sql += " aa.behaviour, aa.testimony,  aa.owner_name,aa.business_name,aa.surety_name, aa.leader_position, aa.guilty, aa.duty, aa.director_position,  "
        sql += " to_char(aa.arrest_report_date,'dd/mm/yyyy', 'NLS_CALENDAR=''THAI BUDDHA'' NLS_DATE_LANGUAGE=THAI') arrest_report_date,  "
        sql += " aa.administration, to_char(aa.finished_time,'HH24:mi') finished_time, ed.indc_off "
        sql += " from application_arrest aa  "
        sql += " left join v_accuser_list_popup sta on sta.idcard_no=aa.accuser_idcardno  "
        sql += " left join title tia on tia.title_code=sta.title_code "
        sql += " left join staff str on str.idcard_no=to_char(aa.report_to) "
        sql += " left join title tir on tir.title_code=str.title_code "
        sql += " left join staff sto on sto.idcard_no=to_char(aa.order_by) "
        sql += " left join title tio on tio.title_code=sto.title_code "
        sql += " inner join legislation lg on lg.id=aa.legislation_id "
        sql += " inner join duty_group dg on dg.group_id=aa.product_group_id "
        sql += " inner join province pv on pv.province_code=aa.scene_province "
        sql += " left join district d on d.district_code=aa.scene_district "
        sql += " left join subdistrict sd on sd.subdistrict_code=aa.scene_subdistrict "
        sql += " inner join ed_office ed on ed.offcode=aa.offcode "
        sql += " left join ed_office eda on eda.offcode=aa.accuser_offcode "
        sql += " left join position ps on ps.poscode=aa.accuser_poscode "
        sql += " left join application_notice_arrest ana on ana.id=aa.application_notice_arrest_id "
        sql += " left join application_search_warrant asw on asw.id=aa.application_search_warrant_id "
        sql += " left join application_sincere_confirm asi on asi.id=aa.application_sincere_confirm_id "
        ...
        If gvArrestLawbreaker.Rows.Count > 0 Then
            Try
                ...
                For i As Integer = 0 To gvArrestLawbreaker.Rows.Count - 1
                    ...
                    Dim sql As String = "insert into application_arrest_lawbreaker(id,update_by,update_on, track_no,culprit_type,lawbreaker_id_card, " & vbNewLine
...
#End Region 'Lawbreaker
    ...
    Private Sub GetLoginHisID(ByVal UserName As String)
        Dim sql As String = "select id "
        sql += " from login_his "
        sql += " where username='" & UserName & "'"
        sql += " and to_char(login_time,'ddmmyyyy')=to_char(sysdate,'ddmmyyyy')"
        sql += " and logout_time is null"
        sql += " and ip_address='" & Request.UserHostAddress & "'"
        sql += " order by id desc"

        Dim dt As New DataTable
        dt = Config.ExcuteTable(sql)
        If dt.Rows.Count > 0 Then
            lblLoginHisID.Text = dt.Rows(0)("id")
        End If
        dt.Dispose()
    End Sub

    Private Sub SaveLogTrans(ByVal UserName As String, ByVal LogMsg As String)
        Dim trans As New TransactionDB
        Try
            LogMsg = LinqDB.Common.Utilities.OracleDB.SetString(LogMsg)
            Dim SessionID As String = LinqDB.Common.Utilities.OracleDB.SetString(Session.SessionID)
            ...
            sql += " values(tran_log_seq.nextval,'" & UserName & "',sysdate," & LogMsg & ",'" & Request.UserHostAddress & "'," & SessionID & ",'" & Browser & "','" & lblLoginHisID.Text & "')"
...
#Region "Arrest Exhibit"
    ...
    Private Sub frmPopArrestExhibit1_SaveExhibit(ByVal sender As Object, ByVal e As System.EventArgs, ByVal GroupID As String, ByVal GroupName As String, ByVal DutyCode As String, ByVal DutyName As String, ByVal IsDomestic As String, ByVal BrandCode As String, ByVal BrandName As String, ByVal SizeDesc As String, ByVal SizeUintCode As String, ByVal SizeUnitName As String, ByVal Qty As String, ByVal QtyUnitCode As String, ByVal QtyUnitName As String, ByVal NetWeight As String, ByVal NetWeightUnitCode As String, ByVal NetWeightUnitName As String, ByVal ExhibitStatus As String, ByVal ExhibitList As String, ByVal ExhibitDesc As String, ByVal CarNo As String, ByVal Remarks As String, ByVal RowID As String) Handles frmPopArrestExhibit1.SaveExhibit
        Dim dt As New DataTable
        dt = GetExhitbitList()
    ...
    Private Sub gvExhibitList_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvExhibitList.RowCommand
        If e.CommandName = "Edit" Then
            ...
            frmPopArrestExhibit1.ShowPopEdit(RowID, GroupID, DutyCode, IsDomestic, BrandCode, BrandName, SizeDesc, SizeUnitCode, Qty, QtyUnitCode, NetWeight, NetWeightUnitCode, ExhibitStatus, ExhibitList, ExhibitDesc, CarNo, Remarks)
        End If
    End Sub
