    ...
    Public Sub Update(ByVal rowCount As Long)
        If gvMain IsNot Nothing Then
            If gvMain.PageCount > 0 Then
                ...
                lblTotalPage.Text = gvMain.PageCount.ToString(IntFormat)
                ...
                lblSummary.Text = "Row " & ((gvMain.PageIndex * gvMain.PageSize) + 1).ToString(IntFormat) & " - " & ((gvMain.PageIndex * gvMain.PageSize) + gvMain.Rows.Count).ToString(IntFormat) & " From " & rowCount.ToString(IntFormat) & " Row(s)"
                txtPageIndex.Text = "0"
            Else
                Me.Visible = False
            End If
    ...
    Public Sub Update(ByVal curPage As Integer, ByVal total As String, ByVal pageSize As Integer)
        ...
        If gvMain IsNot Nothing Then
            ...
            If pageCount > 0 Then
                ...
                lnbBack.Enabled = (curPage <> 0)
                lnbNext.Enabled = (curPage < gvMain.PageCount - 1)
                lblSummary.Text = "รายการที่ " & ((gvMain.PageIndex * gvMain.PageSize) + 1).ToString(IntFormat) & " - " & ((gvMain.PageIndex * gvMain.PageSize) + gvMain.Rows.Count).ToString(IntFormat) & " จาก " & total & " รายการ"
            Else
                Me.Visible = False
            End If
        End If
