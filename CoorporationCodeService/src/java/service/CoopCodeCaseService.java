...
public class CoopCodeCaseService extends BaseService {
    ...
    public responseHeader getResponse(requestHeader reqHeader) throws Exception{
        ...
        if(validateHeader(reqHeader)==true){
           if(isValidData(reqHeader.getCoorporation_code(), reqHeader.getRestrospect_year())==true){
               ret.setIsValidData(true);
               ret.setMessage("");
               ret.setResponseBody(setCoopCodeCase(reqHeader.getCoorporation_code(), reqHeader.getRestrospect_year()));
           }else{
    ...
    private ArrayList<CoopCodeCase> setCoopCodeCase(String coorporation_code, long restrospect_year) throws Exception{
        ArrayList ret = new ArrayList();
        try{
            BaseDomain aa = new BaseDomain(db);
            String sql = "";
            sql += " select aal.corporation_code coorporation_code, tc.title_code company_title_code, tc.title_name company_title_name, aal.company_name, ";
            ...
            sql += " left join title tc on tc.title_code=aal.company_title_code ";
            ...
            sql += " where aal.corporation_code ='" + coorporation_code + "' ";
            ...
            for(int i=0;i<lList.size();i++){
                CoopCodeCase data = new CoopCodeCase();
                Map<String,Object> map = lList.get(i);

                data.setCoorporation_code(coorporation_code);
                data.setCompany_title_code((map.get("company_title_code") != null ? map.get("company_title_code").toString() : ""));
                data.setCompany_title_name((map.get("company_title_name") != null ? map.get("company_title_name").toString() : ""));
                data.setCompany_name((map.get("company_name") != null ? map.get("company_name").toString() : ""));
                ...
                //Set ExhibitListInfo
                String eSql = "select aae.id, nvl(aae.group_id,aae.duty_code) duty_code, nvl(dg.group_name, dt.duty_name) duty_name, aae.product_code, nvl(dt.duty_name,dg.group_name) product_name, ";
                eSql += " aae.brand_code, nvl(bm.brand_main_thai,nvl(bm.brand_main_eng,aae.brand_name)) brand_name ,aae.isdomestic, ";
                eSql += " aae.sizes_code, aae.sizes_desc, aae.size_unit_code, nvl(su.thname,aae.sizes_unit_name) sizes_unit_name, ";
                ...
                //eSql += " left join size_package sp on sp.size_code=aae.sizes_code and sp.duty_code=aae.duty_code ";
    ...
    private boolean validateHeader(requestHeader reqHeader) throws Exception {
        ...
        if(reqHeader.getAppID() == null || reqHeader.getAppID().trim().equals("")){
            ...
            ret = false;
        }else if(reqHeader.getCoorporation_code() == null || reqHeader.getCoorporation_code().trim().equals("")){
            _err = "กรุณาระบุเลขเลขทะเบียนนิติบุคคล";
            ret = false;
    ...
    private boolean isValidData(String coorporation_code, long restrospect_year) throws Exception{
        boolean ret = false;
        try{
            BaseDomain aa = new BaseDomain(db);
            ...
            sql += " where upper(trim(aal.corporation_code)) = '" + coorporation_code + "' ";
            sql += " and to_char(bi.case_date,'yyyy') >= (to_char(sysdate,'yyyy') - " + restrospect_year + ")";
//System.out.println(sql);
            List<Map<String,Object>> lList = aa.findBySql(sql);
            if(lList.size()>0)
	...
	public static void main(String args[]) throws Exception {
            CoopCodeCaseService cs = new CoopCodeCaseService();
            ArrayList<CoopCodeCase> lList = cs.setCoopCodeCase("ABC",10);

            for(int i=0;i<lList.size();i++){
                System.out.println(lList.get(i).getTrack_no().toString());
                System.out.println(lList.get(i).getCompany_title_code().toString());
                System.out.println(lList.get(i).getCompany_title_name().toString());
                System.out.println(lList.get(i).getCompany_name().toString());
