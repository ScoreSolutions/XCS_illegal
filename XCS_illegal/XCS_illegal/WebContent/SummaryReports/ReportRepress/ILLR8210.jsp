		...
		<tr>
			<td width="100%" align="left" valign="top">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="1%">&nbsp;</td>
		            	<td width="99%" height="40"><span class="websitename">2.10 รายงานผลงานนายตรวจ</span></td>
		            </tr>   
					...
					function report(){
					 ...
					 var dateto;
						...
						if(compare_case_date==""||compare_case_date1==""){
							...
							if(sector_code==""){
								var param = '&vDatefrom='+datefrom+'&vDateto='+dateto+'&vUserID='+user_id + '&time=' + <%= new java.util.Date().getTime() %>;
								Reports.ReportUrl(function(data){
									if(data != ""){
										window.open(data + 'ReportName=ILLR8210_1&vReportType=PDF' + param, '_blank' , 'menubar=no,toolbar=yes,resizable=yes,location=yes,scrollbars=no,width=900,height=700');
									}else{
										window.open ('process?action=Report&__report=ILLR8210_1&__formatt=pdf'+ param, '_blank' ,'menubar=no,toolbar=yes,resizable=yes,location=yes,scrollbars=no,width=900,height=700');
									}
								});
							}else{
								var param = '&vDatefrom='+datefrom+'&vDateto='+dateto+'&vOffcode1='+sector_code+'&vOffcode2='+area_code+'&vOffcode3='+branch_code+'&vUserID='+user_id + '&time=' + <%= new java.util.Date().getTime() %>;
								Reports.ReportUrl(function(data){
									if(data != ""){
										window.open(data + 'ReportName=ILLR8210&vReportType=PDF' + param, '_blank' , 'menubar=no,toolbar=yes,resizable=yes,location=yes,scrollbars=no,width=900,height=700');
									}else{
										window.open ('process?action=Report&__report=ILLR8210&__formatt=pdf' + param, '_blank' ,'menubar=no,toolbar=yes,resizable=yes,location=yes,scrollbars=no,width=900,height=700');
									}
								});
							}
						}
