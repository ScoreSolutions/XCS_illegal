...
<script type='text/javascript' src='dwr/util.js'></script>
	...
	<td width="100%" align="left" valign="top">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr>
					<td width="1%">&nbsp;</td>
            		<td width="99%" ><span class="websitename">3.3 ผลสถิติ (ผลการปราบปรามหรือผู้กระทำผิด) ส่วนคดีสำนักกฎหมาย</span></td>
            	</tr> 
...
<script type='text/javascript'>
	...
	function report(){
		...
		if(compare_case_date==""||compare_case_date1==""){
			...
			if(sector_code==""){
			//	alert ('process?action=Report&__report=ILLR8303_1&__formatt=pdf&getDatefrom='+datefrom+'&getDateto='+dateto+'&getUserID='+user_id, '_blank' ,'menubar=no,toolbar=yes,resizable=yes,location=no,scrollbars=no,width=900,height=700');
				Reports.ReportUrl(function(data){
					if(data != ""){
						window.open(data + 'ReportName=ILLR8303_1&vReportType=PDF&getDatefrom='+datefrom+'&getDateto='+dateto+'&getUserID='+user_id + '&time=' + Math.random(),
								'_blank' ,'menubar=no,toolbar=yes,resizable=yes,location=no,scrollbars=no,width=900,height=700');
					}else{
						window.open('process?action=Report&__report=ILLR8303_1&__formatt=pdf&getDatefrom='+datefrom+'&getDateto='+dateto+'&getUserID='+user_id + '&time=' + Math.random(), 
								'_blank' ,'menubar=no,toolbar=yes,resizable=yes,location=no,scrollbars=no,width=900,height=700');
					}
				});	
			}else{
			...
			//	'&getOffcode1='+sector_code+'&getOffcode2='+area_code+'&getOffcode3='+branch_code+'&getUserID='+user_id, '_blank' ,'menubar=no,toolbar=yes,resizable=yes,location=no,scrollbars=no,width=900,height=700');
				Reports.ReportUrl(function(data){
					if(data != ""){
						window.open(data + 'ReportName=ILLR8303&vReportType=PDF&getDatefrom='+datefrom+'&getDateto='+dateto+
