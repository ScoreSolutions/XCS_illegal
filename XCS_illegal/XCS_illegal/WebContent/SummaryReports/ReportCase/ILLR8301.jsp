...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<td width="100%" align="left" valign="top">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr>
					<td width="1%">&nbsp;</td>
            		<td width="99%" ><span class="websitename">3.1 รายงานผลคดี ส.ส.2/55</span></td>
            	</tr> 
				...
				<tr>
		          		...
		          		<table width="700" border="0" cellspacing="0" cellpadding="0">
		           			<tr>
		           				...
		           				<td align="left" width="80%">
		           					<select id="sector_code" name="sector_code" style="width: 325px" onchange="getArea('area_code',this.value),getBranch('branch_code',this.value)" <c:if test="${IsCenter != 'Y'}" >disabled</c:if> ></select>
		           				</td>
		           			</tr>
		           			<tr>
								<td align="right" height="25px">พื้นที่ :&nbsp;</td>
								<td>  
									<select id="area_code" name="area_code" style="width: 325px" onchange="getBranch('branch_code',this.value)" <c:if test="${IsSection != 'Y' && IsCenter != 'Y'}" >disabled</c:if> ></select>
				                </td>
							</tr>
							<tr>
								<td align="right" height="25px"> พื้นที่/สาขา :&nbsp;</td>
								<td>
									<select id="branch_code" name="branch_code" style="width: 325px" <c:if test="${IsArea != 'Y' && IsSection != 'Y' && IsCenter != 'Y'}" >disabled</c:if> /></select>
				        		</td>
							</tr>
							<tr>
								<td height="30px">&nbsp;</td>
								<td>
									...
									&nbsp;&nbsp;&nbsp;
									<input name="btnReportExcel" type="button" class="barbutton" value="รายงาน Excel" onClick="report('EXCEL');"/>
								</td>
							</tr>
		             	</table>
		          	</td>
              	 ...
              	 <script type='text/javascript'>
					 ...
					 var dateto;
						...
						if(compare_case_date==""||compare_case_date1==""){
							alert('กรุณาป้อนวันที่ทำรายงาน');
   							...
   							//alert ('process?action=Report&__report=ILLR8301&__formatt=pdf&getDatefrom='+datefrom+'&getDateto='+dateto+
							//'&getOffcode1='+sector_code+'&getOffcode2='+area_code+'&getOffcode3='+branch_code+'&getUserID='+user_id, '_blank' ,'menubar=no,toolbar=yes,resizable=yes,location=no,scrollbars=no,width=900,height=700');
							
							Reports.ReportUrl(function(data){
								var ReportName = "ILLR8301";
								if (vReportType=="EXCEL")
									ReportName="ILLR8301_EXCEL";
								
								if(data != ""){
									window.open(data + 'ReportName='+ ReportName +'&vReportType='+ vReportType +'&getDatefrom='+datefrom+'&getDateto='+dateto+
									...
									window.open('process?action=Report&__report=ILLR8301&__formatt='+ vReportType +'&getDatefrom='+datefrom+'&getDateto='+dateto+
									'&getOffcode1='+sector_code+'&getOffcode2='+area_code+'&getOffcode3='+branch_code+'&getUserID='+user_id + '&time=' + Math.random(), 
									'_blank' ,'menubar=no,toolbar=yes,resizable=yes,location=no,scrollbars=no,width=900,height=700');
								}
							});
					...
					var vOffcode = "${vOffcode}";  //100100
					if (vOffcode != ""){
						if (IsCenter != "Y"){
							var OffcodeSection = vOffcode.substring(0,2) + "0000";
							var OffcodeArea = vOffcode.substring(0,4) + "00";
							var OffcodeBranch = vOffcode;
	
							getSector('sector_code',OffcodeSection);
							getArea('area_code',OffcodeArea);
							getBranch('branch_code',OffcodeBranch);
						}else{
