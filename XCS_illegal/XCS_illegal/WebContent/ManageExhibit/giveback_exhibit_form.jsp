...
<script language="javascript">
	...
	var unit_desc='';
	var action_form =  'ExhibitReturn';
	var module_name = 'การคืนของกลาง ';
	var so_id = '${so.id}';
	var jsonUnit = eval('${unit_json}');
	function chkReturnDuplicate(){
		var chk_code = dwr.util.getValue("stockout_code");
		
		Exhibit.chkDupStockout(chk_code, function(data) {
	    	if(data != ""){
	    		alert("เลขที่เอกสาร กค. ซ้ำ");
	    		document.formdata1.stockout_code.value = "";
	    		document.formdata1.stockout_code.focus();
	    	}		    	
	  	});
	}
...
<form action="process?action=ExhibitReturn&cmd=update" name="formdata1" method="post">
			...
			<tr>
				...
				<td>
					...
					<c:if test="${so.status!='2' }">
						...
						<c:choose>
							<c:when test="${so.id != null}">
								<input name="btnCancel" type="button" class="barbutton" value="ยกเลิกการแก้ไข" onclick="javascript:clickReset();">
								<input name="btnBack" type="button" class="barbutton" value="กลับหน้าค้นหา" onclick="javascript:clickBack();">
								<input name="btnConfirm" type="button" class="barbutton" value="ยืนยัน" onclick="javascript:clickConfirm();">
							</c:when>
							<c:otherwise>
								<input name="btnCancel" type="reset" class="barbutton" value="ยกเลิกการแก้ไข">
								<input name="btnBack" type="button" class="barbutton" value="กลับหน้าค้นหา" onclick="javascript:clickBack();">
							</c:otherwise>
						</c:choose>
					</c:if>
			...
			<tr>
				...
				<table width="750" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td height="25" align="right">เลขที่เอกสาร ก.ค. :&nbsp;</td>
								<td>
									<input type="text" name="stockout_code" class="textbox" size="5" maxlength="9" value="${so.stockoutCode1}" onblur="javascript:chkReturnDuplicate();"/> 
				...
				<tr>
					...
					<table id="exhibit_table" class="tableList" width="1000" border="0" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC">
						<tr>
							...
							<td width="10%" class="TblHeaderColumn AlignCenter">จำนวนที่คืนของกลาง</td>
						...
						<tr>
							...
							<td><div >
								<input type='text' name='qty' size='3' value='${soi.qty}' class='textNum' 
						...
						<tr>
							<td colspan="7" align="right"><b><u>รวมจำนวนที่คืนของกลาง :&nbsp;</u></b></td>
...
<script language="JavaScript" type="text/javascript">
	//You should create the validator only after the definition of the HTML form
  	...
  	frmvalidator.addValidation("stockout_code","req","กรุณากรอกเลขที่ใบเสร็จ");
