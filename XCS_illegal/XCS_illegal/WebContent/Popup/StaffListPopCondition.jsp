	...
	for (var i=1; i < document.dataform3.thecheckbox.length; i++)
	{
		if (document.dataform3.thecheckbox[i].checked)
		{
			...
			data[count]=new Array();
				...
				data[count][4]=document.dataform3.empid[i].value;
				...
				//alert("data[count][1]"+data[count][1]);
				//alert("data[count][2]"+data[count][2]);
				//alert("data[count][3]"+data[count][3]);
				//alert("data[count][4]"+data[count][4]);
				//alert("data[count][5]"+data[count][5]);
				//alert("data[count][6]"+data[count][6]);
				//alert("data[count][7]"+data[count][7]);
				//alert("data[count][8]"+data[count][8]);
				//alert("data[count][9]"+data[count][9]);
				//alert("data[count][10]"+data[count][10]);
				//alert("data[count][11]"+data[count][11]);
				//alert("data[count][12]"+data[count][12]);
				//alert("data[count][13]"+data[count][13]);
...
<BODY>
<form method="post" name="dataform" action="<c:url value='process?action=StaffListPopCondition&cmd=list'/>">
<table border="0" cellpadding="0" cellspacing="0" width="95%">
	...
	<tr>
		...
		<td >
			<input name="txtoffname" value="${offname}" type="text" class="textview" readonly  size="50">
			<input name="txtoffcode" value="${offcode}" type="hidden" class="text"  >
		</td>
	</tr>
	<tr>
		<td align="right">&nbsp;</td>
...
<table class="tableList" width="95%" border="0" cellspacing="0" cellpadding="0" bordercolor="#CCCCCC">
       ...
       <tr>
			...
			<td width="25%" class="TblHeaderColumn">ชื่อ-สกุล
				<input type="hidden" id="fullname" name="fullname" value="" /></td>
			<td width="20%" class="TblHeaderColumn">ตำแหน่ง
	  ...
	  <c:forEach items="${divide_staffList}"  var="staff" varStatus="i">
			...
			<td align="left" valign="top" class="TblRow AlignLeft"><span class="TblRow AlignLeft">${staff.offname}
				...
				<input type="hidden" id="empid" name="empid" value="${staff.empid}" />
				...
				<input type="hidden" id="staff_level" name="staff_level" value="${staff.staff_level}" /></span></td>
			</tr>
			</c:if>
