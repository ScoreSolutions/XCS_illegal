...
<HEAD>
    ...
    <script language="javascript">
        ...
        function initValue(flag) {
            if (flag) {
                writeTranLog('ยกเลิกการแก้ไขที่อยู่ผู้กระทำผิด');
            }
        <c:if test="${sa != null}">
        ...
        <%--jQuery("[name=address]").val('${sa.address}');--%>
            if (row >= 0) {
                ...
                jQuery("[name=fax_no]").val(opener.document.dataform.lawbreaker_address_fax_no[row].value);
                jQuery("[name=address_name]").val(opener.document.dataform.lawbreaker_address_address_name[row].value);
                jQuery("[name=moo]").val(opener.document.dataform.lawbreaker_address_moo[row].value);
                jQuery("[name=road]").val(opener.document.dataform.lawbreaker_address_road[row].value);
                jQuery("[name=soi]").val(opener.document.dataform.lawbreaker_address_soi[row].value);
                jQuery("[name=zipcode]").val(opener.document.dataform.lawbreaker_address_zipcode[row].value);
                getProvince('sProvince', 'sDistrict', 'sSubDistrict',opener.document.dataform.lawbreaker_address_province_code[row].value
                        ,opener.document.dataform.lawbreaker_address_district_code[row].value,opener.document.dataform.lawbreaker_address_subdistrict_code[row].value);
                jQuery("[name=coordinate_x]").val(opener.document.dataform.lawbreaker_address_coordinate_x[row].value);
                jQuery("[name=coordinate_y]").val(opener.document.dataform.lawbreaker_address_coordinate_y[row].value);
                jQuery("[name=description]").val(opener.document.dataform.lawbreaker_address_description[row].value);
                jQuery("[name=address]").val(opener.document.dataform.lawbreaker_address_address[row].value);
                }else{
                    jQuery("[name=tel_no]").val(opener.document.dataform.lawbreaker_address_tel_no.value);
                    jQuery("[name=fax_no]").val(opener.document.dataform.lawbreaker_address_fax_no.value);
                    jQuery("[name=address_name]").val(opener.document.dataform.lawbreaker_address_address_name.value);
                    jQuery("[name=moo]").val(opener.document.dataform.lawbreaker_address_moo.value);
                    jQuery("[name=road]").val(opener.document.dataform.lawbreaker_address_road.value);
                    jQuery("[name=soi]").val(opener.document.dataform.lawbreaker_address_soi.value);
                    jQuery("[name=zipcode]").val(opener.document.dataform.lawbreaker_address_zipcode.value);
                    getProvince('sProvince', 'sDistrict', 'sSubDistrict',opener.document.dataform.lawbreaker_address_province_code.value
                            ,opener.document.dataform.lawbreaker_address_district_code.value,opener.document.dataform.lawbreaker_address_subdistrict_code.value);
                    jQuery("[name=coordinate_x]").val(opener.document.dataform.lawbreaker_address_coordinate_x.value);
                    jQuery("[name=coordinate_y]").val(opener.document.dataform.lawbreaker_address_coordinate_y.value);
                    jQuery("[name=description]").val(opener.document.dataform.lawbreaker_address_description.value);
                    jQuery("[name=address]").val(opener.document.dataform.lawbreaker_address_address.value);
                }
            }
            <%--else {--%>
                <%--jQuery("[name=tel_no]").val('${sa.tel_no}');--%>
        ...
        function appendRow() {
            ...
            if (checkValidate()) {
                ...
                data["full_address"] = q(data["address_name"], "") +
						(data["address"] ? " เลขที่ " + data["address"] : "") +
						(data["moo"] ? " หมู่ " + data["moo"] : "") +
						(data["subdistrict_text"] != "โปรดเลือก" ? " ตำบล" + data["subdistrict_text"] : "") +
						(data["district_text"] != "โปรดเลือก" ? " อำเภอ" + data["district_text"] : "") +
						(data["province_text"] != "โปรดเลือก" ? " จังหวัด" + data["province_text"] : "");
                ...
                writeTranLog('บันทึกที่อยู่ผู้กระทำผิด');
        ...
        function close_form() {
            writeTranLog('ปิดหน้าจอเพิ่มที่อยู่ผู้กระทำผิด');
            window.close();
        }
    </script>
