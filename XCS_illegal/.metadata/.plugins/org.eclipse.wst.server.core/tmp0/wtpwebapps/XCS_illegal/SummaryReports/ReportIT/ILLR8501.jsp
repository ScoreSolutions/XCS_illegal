	...
	<tr>
		<td width="100%"align="left" valign="top">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
		 		<tr>
		 			<td width="1%" >&nbsp;</td>
         			<td width="99%"><span class="websitename">5.1 ตารางแสดงผลการปราบปรามผู้กระทำผิด แยกตามพื้นที่ (รายปี)</span></td>
         		</tr> 
              	...
              	<script type='text/javascript'>
					...
					function report(){
						...
						if(monthto==""||monthfrom==""){
							...
							if(sector_code==""){
								var param = '&getDatefrom='+datefrom+'&getDateto='+dateto+'&getYear='+year1+'&getUserID='+user_id  + '&time=' + Math.random();
								Reports.ReportUrl(function(data){
									if(data != ""){
										window.open(data + 'ReportName=ILLR8501&vReportType=PDF' + param, '_blank' , 'menubar=no,toolbar=yes,resizable=yes,location=no,scrollbars=no,width=900,height=700');
									}else{
										window.open('process?action=Report&__report=ILLR8501&__formatt=pdf' + param,'_blank' , 'menubar=no,toolbar=yes,resizable=yes,location=no,scrollbars=no,width=900,height=700');
									}
								});
							}
							else{
								var param = '&getDatefrom='+datefrom+'&getDateto='+dateto+'&getYear='+year1+'&getOffcode='+sector_code+'&getUserID='+user_id+ '&time=' + Math.random();
								Reports.ReportUrl(function(data){
									if(data != ""){
										window.open(data + 'ReportName=ILLR8501_1&vReportType=PDF' + param, '_blank' , 'menubar=no,toolbar=yes,resizable=yes,location=no,scrollbars=no,width=900,height=700');
									}else{
										window.open('process?action=Report&__report=ILLR8501_1&__formatt=pdf' + param,'_blank' , 'menubar=no,toolbar=yes,resizable=yes,location=no,scrollbars=no,width=900,height=700');
									}
								});
